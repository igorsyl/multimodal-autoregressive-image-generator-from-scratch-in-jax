...
def generate_toy_images(key, num_images, image_size):
    # return num_images grayscale images, each a bright square on a zero background
    square_size = image_size//2
    max_idx = image_size - square_size
    def single(subkey):
        k1, k2 = jax.random.split(subkey)
        top_left_row = jax.random.randint(k1, shape=(), minval=0, maxval=max_idx+1)
        top_left_col = jax.random.randint(k2, shape=(), minval=0, maxval=max_idx+1)
        img = jnp.zeros((image_size, image_size))
        patch = jnp.ones((square_size, square_size))
        img_with_square = jax.lax.dynamic_update_slice(img, patch, (top_left_row, top_left_col))
